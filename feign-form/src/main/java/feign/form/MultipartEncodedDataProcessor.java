...
public class MultipartEncodedDataProcessor implements FormDataProcessor {
    ...
    try {
      ...
      for (Map.Entry<String, Object> entry : data.entrySet()) {
        ...
        if (isPayload(entry.getValue())) {
          ...
          writeParameter(writer, entry.getKey(), entry.getValue());
        }
  ...
  protected void writeParameter(PrintWriter writer, String name, Object value) {
    ...
    writer.append(CRLF).append(String.valueOf(value));
  }
   ...
   */
  @SneakyThrows
  protected void writeFile(OutputStream output, PrintWriter writer, String name, String contentType,
                           File file) {
